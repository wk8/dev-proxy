...
func TestProxy(t *testing.T) {
	...
	t.Run("when the transformer errors out",
		func(t *testing.T) {
			...
			if string(body) != "Internal dev-proxy error: dummy error\n" {
				t.Errorf("Unexpected body: %#v", string(body))
			}
	...
	t.Run("when the backend fails to connect before the connect timeout expires",
		func(t *testing.T) {
			...
			if string(body) != "Internal dev-proxy error: Get http://10.255.255.1/: dial tcp 10.255.255.1:80: i/o timeout\n" {
				t.Errorf("Unexpected body: %#v", string(body))
			}
	...
	t.Run("when the backend fails to reply before the global timeout expires",
		func(t *testing.T) {
			...
			if !strings.HasPrefix(string(body), "Internal dev-proxy error: Get http://localhost:"+httpServerPortAsStr+"/sleep_25_ms: net/http: request canceled") {
				t.Errorf("Unexpected body: %#v", string(body))
			}
